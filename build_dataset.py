...
COUNTS = {
    ...
    "mnist": {"train": 60000, "test": 10000, "valid": 5000, "extra": 0},  
    "imagenet_32": {
        "train": 1281167,
        "test": 50000,
        "valid": 50050,
...
def split_l_u(train_set, n_labels):
    ...
    for c in classes:
        ...
        u_labels2 += [c_labels[n_labels_per_cls:]] 
    l_train_set = {"images": np.concatenate(l_images, 0), "labels": np.concatenate(l_labels, 0)}
    u_train_set = {"images": np.concatenate(u_images, 0), "labels": np.concatenate(u_labels, 0),"labels2":np.concatenate(u_labels2, 0)}
...
if args.dataset == "svhn":
    ...
    train_set, test_set = _load_cifar100()
    train_set["images"] = gcn(train_set["images"])
    test_set["images"] = gcn(test_set["images"])
    mean, zca_decomp = get_zca_normalization_param(train_set["images"])
    train_set["images"] = zca_normalization(train_set["images"], mean, zca_decomp)
...
np.save(os.path.join(_DATA_DIR, args.dataset, "l_train_{}".format(args.nlabels)), l_train_set)
np.save(os.path.join(_DATA_DIR, args.dataset, "u_train_{}".format(args.nlabels)), u_train_set)
np.save(os.path.join(_DATA_DIR, args.dataset, "val_{}".format(args.nlabels)), validation_set)
np.save(os.path.join(_DATA_DIR, args.dataset, "test_{}".format(args.nlabels)), test_set)
